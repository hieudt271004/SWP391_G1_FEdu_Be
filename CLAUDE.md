...
- `fedu/src/main/resources/db/migration/` — Flyway migrations. `spring.jpa.hibernate.ddl-auto` is `none`, so the schema comes from Flyway, **not** from Hibernate: `V1__baseline_schema.sql` is the frozen baseline (never edit — Flyway checksums it), and every entity/column change needs a new `V{n}__*.sql`. `baseline-on-migrate` is enabled so pre-existing databases are stamped at V1 untouched. `db/demo/afterMigrate__demo_data.sql` seeds demo data (accounts, subjects, template paths) but only under the `demo` profile and only into an empty database.
...
- **Database Integrity**: Since `ddl-auto` is `none`, any database schema modification must ship as a new Flyway migration `V{n}__*.sql` in `fedu/src/main/resources/db/migration/` (never edit an applied migration — checksums). Ensure custom JPA entities map exactly to their SQL definition. Do not use named Postgres enums.
